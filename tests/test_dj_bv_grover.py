...
def test_dj_constant_oracle(backend, n_qubits):
    ...
    results = deutsch_jozsa(n_qubits=n_qubits, oracle_type="constant", backend=backend, seed=42)
...
def test_dj_balanced_oracle(backend, n_qubits):
    ...
    results = deutsch_jozsa(n_qubits=n_qubits, oracle_type="balanced", backend=backend, seed=42)
...
def test_bv_finds_secret_string(backend):
    ...
    n_qubits = 4
    secret = "1011"
    results = bernstein_vazirani(n_qubits=n_qubits, secret_string=secret, backend=backend, seed=42)
...
def test_grover_finds_marked_item(backend):
    ...
    n_qubits = 3
    marked_item = "110"
    results = grover(n_qubits=n_qubits, marked_item=marked_item, backend=backend, seed=42)
